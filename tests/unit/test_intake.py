...
from autopilot.intake import generate_backlog_items, _slugify
...
def test_generate_backlog_items_parses_clean_json():
    ...
    with patch("autopilot.intake.call_with_fallback", return_value=fake_llm):
...
def test_generate_backlog_items_raises_on_bad_json():
    with patch("autopilot.intake.call_with_fallback", return_value="not json"):
        try:
